...
export type UserStatus = 'EMAIL_NOT_CONFIRMED' | 'PENDING' | 'ACTIVE' | 'BLOCKED' | 'REJECTED';
...
export class UserService {
	...
	getAll(filters: Required<UsersFilterDto>, pagination: UsersPagination): Observable<UsersResponseDto> {
		return this.httpClient.get<UsersResponseDto>(`${this.configService.serverUrl}/v1/users/all`, {
	...
	blockUser(userId: number): Observable<void> {
		return this.httpClient.put<void>(`${this.configService.serverUrl}/v1/users/block`, {
	...
	unblockUser(userId: number): Observable<void> {
		return this.httpClient.put<void>(`${this.configService.serverUrl}/v1/users/unblock`, {
			currentUserId: this.currentUser$.value?.id,
			validData: true,
			userId,
		});
	...
	rejectUser(userId: number): Observable<void> {
		return this.httpClient.put<void>(`${this.configService.serverUrl}/v1/users/${userId}/reject`, null);
	}

	activateUser(userId: number): Observable<void> {
		return this.httpClient.put<void>(`${this.configService.serverUrl}/v1/users/status`, {
			userIds: [userId],
			status: 'ACTIVE',
		});
	}
